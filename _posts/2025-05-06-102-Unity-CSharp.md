...
오, 학생들에게 Unity와 C#을 가르치시는군요! 8차시 동안 C#의 핵심을 다루면서 Unity 개발의 기초를 다질 수 있는 강의 교안을 제안해 드립니다. 이론 설명과 함께 학생들이 직접 코드를 작성하고 결과를 확인할 수 있는 샘플 예제를 각 차시별로 구성했습니다.

**전반적인 강의 목표:**

* C# 프로그래밍의 기본적인 개념과 문법을 이해하고 사용할 수 있다.
* Unity 환경에서 C# 스크립트를 작성하고 게임 오브젝트를 제어할 수 있다.
* 간단한 상호작용 기능을 구현할 수 있는 능력을 배양한다.
...
* **이론:**
    * C#이란 무엇인가? (특징, 장점, 활용 분야)
    * .NET Framework 및 .NET (간단한 소개)
    * Visual Studio Code 및 Unity Hub 설치 및 설정 (실습 환경 구축)
    * C# 프로젝트 생성 및 기본 구조 이해
    * Console 출력 ( `Console.WriteLine()` )
    * 주석 (한 줄 주석 `//`, 여러 줄 주석 `/* ... */`)
* **샘플 예제:**
    * "Hello, World!"를 출력하는 간단한 콘솔 애플리케이션 만들기
    * 이름, 나이 등을 변수에 저장하여 출력하는 프로그램 만들기

**2차시: 변수와 자료형 - 데이터 저장하고 다루기**

* **이론:**
    * 변수 (선언, 할당, 초기화)
    * 기본 자료형 (정수형: `int`, `long`, 실수형: `float`, `double`, 문자형: `char`, 논리형: `bool`, 문자열: `string`)
    * 자료형 변환 (암시적 형변환, 명시적 형변환 - 캐스팅)
    * 상수 ( `const` 키워드)
* **샘플 예제:**
    * 원의 반지름을 입력받아 넓이와 둘레를 계산하는 프로그램 ( `Math.PI` 활용)
    * 두 정수를 입력받아 사칙연산을 수행하고 결과를 출력하는 프로그램
...
* **이론:**
    ...
    * 연산자 우선순위
* **샘플 예제:**
    * 사용자로부터 두 수를 입력받아 크기를 비교하고 결과를 출력하는 프로그램
    * 윤년 판별 프로그램 (4로 나누어 떨어지고 100으로 나누어 떨어지지 않거나 400으로 나누어 떨어지는 해)

**4차시: 조건문 - 프로그램의 흐름 제어하기 (if, else, switch)**

* **이론:**
    * `if` 문 (단일 조건, `if-else` 문, `if-else if-else` 문)
    * `switch` 문 ( `case`, `default`, `break` )
* **샘플 예제:**
    * 점수를 입력받아 학점을 출력하는 프로그램 (`if-else if-else` 활용)
    * 요일을 숫자로 입력받아 해당하는 요일 이름을 출력하는 프로그램 (`switch` 활용)


**강의 진행 팁:**

* 각 차시별 이론 설명 후에는 반드시 학생들이 직접 코드를 작성하고 실행해 보도록 지도해 주세요.
* 샘플 예제 외에도 학생들의 이해도를 높이기 위한 추가적인 연습 문제를 제공하는 것이 좋습니다.
* 학생들이 어려움을 느끼는 부분은 다시 한번 설명하고 개별적인 질문에 성심껏 답변해 주세요.
* 마지막 차시에는 학생들이 배운 내용을 바탕으로 간단한 Unity 프로젝트를 직접 만들어 보도록 유도하는 것도 좋은 방법입니다. (예: 키보드로 움직이는 간단한 오브젝트)

이 교안이 학생들의 C# 학습과 Unity 개발 입문에 도움이 되기를 바랍니다! 궁금한 점이 있다면 언제든지 다시 질문해주세요.