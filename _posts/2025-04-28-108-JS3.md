...
// 3. Rejected (거부): 실패함
//생성자 함수에 전달되는 첫번째 매개변수는 성공시 실행함수, 두번재 매개변수는 실패시 실행함수
...
promise
  .then(result => console.log(result)) // "성공!" 출력
...
//.then()은 "Promise가 완료(resolve)된 후 무엇을 할지 사용자가 정의하는 곳"
...
//const promise2 = Promise.resolve(10); // 단축 표기법

Promise.allSettled([
  Promise.resolve('성공 1'), //이미 성공한 Promise 임
  Promise.reject('실패 1'), // 이미 실패한 Promise 임
  Promise.resolve('성공 2') //이미 성공한 Promise 임
])
  .then(results => {
    console.log(results);
    // [
...
// 함수 안에서 await를 쓰려면 함수에 async가 필수
...
async function searchProducts(keyword) {
  const url = `https://dummyjson.com/products/search?q={keyword}&limit=2`;
  const response = await fetch(url);
  const posts = await response.json();
  return posts;
}

//getUsers()를 호출할 때 await가 필요한 이유는 호출하는 환경이 비동기 작업의 완료를 기다리지 않고 다음 코드를 실행하기 때문
const data =  await getUsers();
console.log('data: ',data );

const data2 = await getUser(7);
console.log('data2: ', data2);

const data3 = await searchProducts('phone');
...
```js
//jsonplaceholder는 가상 응답만 (실제 저장 안됨)
...
// 사용
const result = await fetchWithStatus('https://jsonplaceholder.typicode.com/users');
if (result.error) {
